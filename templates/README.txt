...
2. Download Firefox's full installer from https://releases.mozilla.org （e.g. https://releases.mozilla.org/pub/firefox/releases/%INSTALLER_VERSION%%ESR_SUFFIX%/win%BINARY_BITS%/en-US/Firefox%20Setup%20%INSTALLER_VERSION%%ESR_SUFFIX%.exe ) and put it under the `resources` directory.
...
2. Firefoxのフルインストーラを https://releases.mozilla.org からダウンロードし（例： https://releases.mozilla.org/pub/firefox/releases/%INSTALLER_VERSION%%ESR_SUFFIX%/win%BINARY_BITS%/ja/Firefox%20Setup%20%INSTALLER_VERSION%%ESR_SUFFIX%.exe ）、`resources` ディレクトリ内に置きます。
